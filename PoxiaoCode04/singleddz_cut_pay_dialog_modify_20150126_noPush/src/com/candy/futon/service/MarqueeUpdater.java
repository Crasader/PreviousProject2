...
private final static String TAG = "MarqueeUpdater";
	...
	protected void doUpload(Context context) {
		...
		String param = "appId="
				+GameConfig.getInstance().APP_ID+"&&channelId="+ GameConstantsService.getInstance().getEnterId();
		...
		HttpGet target = new HttpGet(LOG_SERVER_URL+"?"+ "channelId="+  GameConstantsService.getInstance().getEnterId()+ "&appId=" + GameConfig.getInstance().APP_ID);
