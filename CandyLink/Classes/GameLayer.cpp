...
void GameLayer::update(float dt){
	if (GAMEDATA::getInstance()->gameState == GAMEDATA::GameState::Playing){
		...
		if (GAMEDATA::getInstance()->gettime() < 0){
			...
			//char *isrelive;
			//if (GAMEDATA::getInstance()->getisreliveinthislevel())
			//{
			//	isrelive = "true";
			//}
			//else
			...
			//	isrelive = "false";
			//}
			//auto eventextend = String::createWithFormat("failed_%s", isrelive);
			//CallAndroidMethod::getInstance()->logevent(event->getCString(), eventextend->getCString(), sessionTime->getCString());
