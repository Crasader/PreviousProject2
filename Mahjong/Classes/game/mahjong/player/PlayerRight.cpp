...
void PlayerRight::playerTurn(){
	Jong* jong = Jong::create();
	jong->showJong(4, -1);
	jong->setPosition(ccp(RIGHT_POS_X, RIGHT_POS_Y - 320));
	this->addChild(jong);
	MoveTo* move = MoveTo::create(0.2f, ccp(RIGHT_POS_X, RIGHT_POS_Y - 340));
...
void PlayerRight::drawPlayedJong(int ctype){
	...
	jong->setPosition(ccp(RIGHT_POS_X, RIGHT_POS_Y - 340));
	this->addChild(jong);
	//贝塞尔移动
	Point startPoint = ccp(RIGHT_POS_X, RIGHT_POS_Y - 340);
