#include "BlockGroupL.h"
#include "GameField.h"


BlockGroupL::BlockGroupL(void)
{
	//设置类型
	m_type = BlockGroupType::BlockGroupType_L;
}
...
//初始化图形
bool BlockGroupL::Init()
{
	return true;
}

//旋转图形(顺时针)
bool BlockGroupL::Rotate(const vector<BlockObject>& blocks)
{
	switch (m_direction)
	{
	case BlockGroupDirection::BGD_0:	//L形 - 尖朝右 --> 尖朝下
		{
			//所有潜在冲突位置的方块
			vector<int> vecRow;
			vector<int> vecCol;
			...
			int cols[] = {first_col, first_col + 1, first_col + 2, first_col};
			for (int i=0; i<4; i++)
			{
		...
		break;
	case BlockGroupDirection::BGD_3:	//尖朝下 --> 尖朝左
		{
			//所有潜在位置的方块
			vector<int> vecRow;
			vector<int> vecCol;
			...
			int cols[] = {first_col, first_col + 1, first_col + 1, first_col + 1};
			for (int i=0; i<4; i++)
			{
				vecRow.push_back(rows[i]);
				vecCol.push_back(cols[i]);
		...
		break;
	case BlockGroupDirection::BGD_2:	//尖朝左 --> 尖朝上
		{
			//所有潜在冲突位置的方块
			vector<int> vecRow;
			vector<int> vecCol;
			...
			int first_col = m_blocks[0].col;
			if (m_blocks[1].col == GameField::GetInstance()->GetBlockColCount() - 1)
			{
				first_col -= 1;
			}
			int rows[] = {first_row, first_row + 1, first_row + 1, first_row + 1};
			int cols[] = {first_col + 2, first_col, first_col + 1, first_col + 2};
			for (int i=0; i<4; i++)
			{
				vecRow.push_back(rows[i]);
				vecCol.push_back(cols[i]);
		...
		break;
	case BlockGroupDirection::BGD_1:	//尖朝上 --> 尖朝右
		{
			//所有潜在位置的方块
			vector<int> vecRow;
			vector<int> vecCol;
			...
			int cols[] = {first_col, first_col, first_col, first_col + 1};
			for (int i=0; i<4; i++)
			{
				vecRow.push_back(rows[i]);
				vecCol.push_back(cols[i]);
...
//加速下落图形
void BlockGroupL::SpeedUp()
{

}

//设置图形类型
void BlockGroupL::SetBlockGroupType()
{
	m_type = BlockGroupType::BlockGroupType_L;
}