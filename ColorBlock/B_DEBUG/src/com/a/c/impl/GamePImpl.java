package com.a.c.impl;

import android.app.Activity;
import android.content.Context;

import com.a.c.base.IGamePay;
import com.d.event.EventCallBack;
import com.d.event.EventResult;
import com.poxiao.pay.xbll.plugin.GamePayDebugImpl;
...
public class GamePImpl extends IGamePay {
	...
	private void setPayPlginByName() {
		setPayPlugin(new GamePayDebugImpl(), activity, false);
	}
	...
	public boolean doPayEvent(final Activity activity, final String eventId,final boolean isShowUi,
		...
		if (null != event) {
			if (event.getEventState()) {
				pay(activity, eventId, isShowUi, false, new PCallback() {
					@Override
					public void result(OrderResultInfo orderResultInfo) {
						EventResult result = new EventResult();
						result.setPResult(orderResultInfo.getResultCode() == OrderResultInfo.P_SUCCESS ? true
								: false);
						// 根据支付结果,返回奖励
