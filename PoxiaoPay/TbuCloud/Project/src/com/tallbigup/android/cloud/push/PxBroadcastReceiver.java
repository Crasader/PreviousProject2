...
public class PxBroadcastReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals("com.avos.UPDATE_STATUS")) {
			try {
				...
				if (packInfo != null || launchInfo != null
						|| contentUrl != null) {
					...
					if (userType != null
							&& !userType.equals("")
							&& !String.valueOf(TbuCloud.markUserType(context))
									.equals(userType)) {
						return;
					}
					...
					if (noLogin != null && !noLogin.equals("")) {
						...
						if ((System.currentTimeMillis() - lastLoginTime) > (Long
								.valueOf(noLogin) * 24 * 60 * 60 * 1000)) {
							return;
						}
					}
					doPush(packInfo, launchInfo, contentUrl, iconUrl, title,
							content, version);
				}
	...
	private void doPush(String packInfo, String launchInfo, String contentUrl,
			final String iconUrl, final String title, final String content,
			String version) {
		...
		if (packInfo != null) {
			if (isPackageInstall(context, packInfo)) {
				...
				if (prizeType != null && !prizeType.equals("-1")) {
					intent.putExtra(ActivityNotifyManager.START_TYPE,
							ActivityNotifyManager.START_BY_ACTIVITY_NOTIFY);
					intent.putExtra(ActivityNotifyManager.NOTITY_ID, id);
					intent.putExtra(ActivityNotifyManager.PRIZE_NUM, prizeNum);
					intent.putExtra(ActivityNotifyManager.PRIZE_TYPE, prizeType);
					intent.putExtra(ActivityNotifyManager.ACTIVITY_CONTENT,
							content);
				}
	...
	private boolean isPackageInstall(Context context, String packinfo) {
		...
		try {
			packageInfo = context.getPackageManager().getPackageInfo(packinfo,
					0);
	...
	public void download(String urlDownload, String version) {
			...
			try {
				...
				if (isSDCardExist()) {
					...
					fileDir = context.getFilesDir().getAbsolutePath()
							+ File.separator + BUFFER_FILE_NAME_PRO + sInfos[1]
							+ ".apk";
				...
				if (isSDCardExist()) {
					...
					output = context.openFileOutput(BUFFER_FILE_NAME_PRO
							+ sInfos[1] + ".apk", Context.MODE_WORLD_READABLE);
				}
				...
				Log.i("POXIAOCLOUD",
						"get error message on download." + e.getMessage());
				successDownload = false;
			}
			return null;
		}
	...
	private void showNotify(final NotificationManager manager,
	...
	private static String getSdFilePath(String version) {
		return (Environment.getExternalStorageDirectory().getAbsolutePath()
				+ File.separator + BUFFER_FILE_NAME_PRO + version + ".apk");
	}
