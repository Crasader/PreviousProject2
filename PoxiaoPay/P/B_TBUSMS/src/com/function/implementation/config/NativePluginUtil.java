package com.function.implementation.config;
...
public class NativePluginUtil {
	private static final String PAY_CONFIG = "json/payconfig.json";// 定义json文件的目录及名称
	...
	public static void initPayConfigInfo(Context context) {
		...
		try {
			// 获取文件内容
			input = context.getAssets().open(PAY_CONFIG);
			...
			try {
				...
				JSONArray arrary = jobject.getJSONArray("payconfig");
				JSONObject object = arrary.getJSONObject(0);
				if (object.has("DEFAULT_PAY_LISTS")) {
					supportPlagins = object.getString("DEFAULT_PAY_LISTS");
				} else {
					Debug.e("NativePluginUtil->init, DEFAULT_PAY_LISTS can not find... ");
					System.exit(0);
				}

				if (object.has("PAY_SEQ_CM")) {
					defaultCMPayPlgin = object.getString("PAY_SEQ_CM");
				} else {
					Debug.e("NativePluginUtil->init, PAY_SEQ_CM can not find... ");
					System.exit(0);
				}
				if (object.has("PAY_SEQ_CU")) {
					defaultCUPayPlgin = object.getString("PAY_SEQ_CU");
				} else {
					Debug.e("NativePluginUtil->init, PAY_SEQ_CU can not load... ");
					System.exit(0);
				}
				if (object.has("PAY_SEQ_CT")) {
					defaultCNPayPlgin = object.getString("PAY_SEQ_CT");
				} else {
					Debug.e("NativePluginUtil->init, PAY_SEQ_CT can not load... ");
					System.exit(0);
				}
				if (object.has("PAY_SEQ_DEFAULT")) {
					defaultNAPayPlgin = object.getString("PAY_SEQ_DEFAULT");
				} else {
					Debug.e("NativePluginUtil->init, PAY_SEQ_DEFAULT can not load... ");
					System.exit(0);
				}
