package com.function.implementation.load;
	...
	private static void readConfig(Context context) {
		...
		try {
			...
			try {
				...
				// 第1段属于setPayPlginByTypeName
				if (jobject.has("setPayPlginByTypeName")) {
					JSONArray pluginList = jobject
							.getJSONArray("setPayPlginByTypeName");
					// 解析setPayPlginByTypeName方法会调用的pluginList的列表
				...
				// 第3段，属于initPayPlginIfNeedOnApplication
				if (jobject.has("initPayPlginIfNeedOnApplication")) {
					JSONArray funcList = jobject
							.getJSONArray("initPayPlginIfNeedOnApplication");
					// 解析setPayPlginByTypeName方法会调用的pluginList的列表
				...
				// 第4段，属于initPayPlginIfNeedOnActivity
				if (jobject.has("initPayPlginIfNeedOnActivity")) {
					JSONArray funcList2 = jobject
							.getJSONArray("initPayPlginIfNeedOnActivity");
					// 解析setPayPlginByTypeName方法会调用的pluginList的列表
