		...
		try {
			...
			try {
				...
				if (jobject.has("applicationImpl")) {
					...
					Debug.e("IocConfigUtil--> readConfig,applicationImpl can not find");
					System.exit(0);
				}
				// 第二段
				if (jobject.has("payImpl")) {
					...
					if (obj.has("className") && obj.has("function")) {
						...
						IocInfos.setPImplFunction(obj.getString("function"));
					} else {
						Debug.e("IocConfigUtil--> readConfig,json is not Compliance with Requirements");
						System.exit(0);
					}
				}
				successInit = true;
			} catch (JSONException e) {
				Debug.e("IocConfigUtil--> readConfig,JSONException:"
						+ e.toString());
				System.exit(0);
			}
		} catch (Exception e) {
			Debug.e("IocConfigUtil-->readConfig,can not load json from asset :"
					+ e.toString());
			System.exit(0);
		} finally {
			if (input != null) {
				try {
					...
					Debug.e("IocConfigUtil-->readConfig, IOException :"
							+ e.toString());
					System.exit(0);
				}
			}
