...
var connectorHost = ''; // 测试服务端连接服务器分配
var connectorPort = '';
...
function initUserList(data) {
	...
	for(var i = 0; i < users.length; i++) {
		...
		slElement.attr("value", users[i].username);
...
$(document).ready(function() {
	...
	pomelo.on('onChat', function(data) {
		...
		if(data.from !== username)
			tip('message', data.from + ';'+connectorHost +';'+connectorPort);
	});

	pomelo.on('onFishes', function(data) {
		addMessage(data.from, data.target, data.msg);
		$("#chatHistory").show();
	});
		...
		queryEntry(username, function(host, port) {
			pomelo.init({
				...
				pomelo.request(route, {
					...
					showChat();
					initUserList(data);
					addMessage('initconfig', '*', data.fishes);
					$("#chatHistory").show();
				});
			});
		});
