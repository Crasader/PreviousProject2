 ...
 */
 function updateAchievementTaskInfoById(client, user_id, task_id, current_level, current_nums, user_get, callback) {
   ...
   client.query(' INSERT INTO gametask_achievement_' + table_index_id +
     ...
     [user_id, task_id, current_level, current_nums, user_get],
     function (err, results) {
         if (err) {
 ...
 * 增加用户任务获取信息
 */
 function addAchievementTaskCurrentNumsById(client, user_id, task_id, add_nums, callback) {
   var table_index_id = 1; // TODO : 后期走配置
   var values = [[user_id, task_id, add_nums]];
   client.query(' INSERT INTO gametask_achievement_' + table_index_id +
      ' (user_id, task_id, current_nums) VALUES ? ' +
 ...
 * 更新用户任务获取信息
 */
 function changeAchievementTaskCurrentNumsById(client, user_id, task_id, add_nums, callback) {
   var table_index_id = 1; // TODO : 后期走配置
   var values = [[user_id, task_id, add_nums]];
   client.query(' INSERT INTO gametask_achievement_' + table_index_id +
      ' (user_id, task_id, current_nums) VALUES ? ' +
      ' ON DUPLICATE KEY UPDATE current_nums = VALUES(current_nums)',
     [values],
     function (err, results) {
         if (err) {
             console.log('[ERROR!!]gametask_achievement_db->changeAchievementTaskCurrentNumsById, msg : ' + err);
             callback(false);
             return;
         }
         callback(true);
...
exports.changeAchievementTaskCurrentNumsById = changeAchievementTaskCurrentNumsById;
